...
SetStudentGradeWindow::SetStudentGradeWindow(QMainWindow * parent ) : QMainWindow (parent) {
    ...
    foreach (Student temp, students ) {
        studentsName.append(QString (temp.getName()+ " " + temp.getFamily())
                            + QString(" (student grade : %1)").arg(QString::number(temp.getGrade())));
    }
    dbManager.close();
...
void SetStudentGradeWindow::onConfirmClicked(){
    DBManager dbManager ;
    ...
    QSqlQuery studentQuery = dbManager.getQuery(
                QString("select id from students where name = '%1' and family = '%2'").arg(
                    temp[0],temp[1]));
    studentQuery.next();
    dbManager.executeCommand(QString("update students set grade=%1 where id=%2").arg(
                                 grade->text(),
